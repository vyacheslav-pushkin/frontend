const rimraf = require('rimraf');
const {runCmdSync} = require('../common');
const fs = require('fs');
const childProcess = require('child_process');

const highlight = '\x1b[34m%s\x1b[0m';
const success = '\x1b[32m%s\x1b[0m';

console.log(highlight, 'Generating API Reference documentation from TSDoc comments...');
rimraf.sync('documentation/api-reference/cuba-rest-js');
rimraf.sync('documentation/api-reference/cuba-react-core');
rimraf.sync('documentation/api-reference/cuba-react-ui');
runCmdSync('lerna run prepublishOnly --scope {@cuba-platform/rest,@cuba-platform/react-core,@cuba-platform/react-ui}');
runCmdSync('lerna run generate-docs');

console.log(highlight, 'Updating main documentation source...');
fs.copyFileSync('README.adoc', `documentation/doc-component-repo/modules/ROOT/pages/index.adoc`);

console.log(highlight, 'Updating navigation panel template...');
childProcess.fork(__dirname + '/update-nav.js').on('exit', () => {
    console.log(success, 'Documentation sources has been updated. Please commit your changes.');
});